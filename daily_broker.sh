...
# 「分點日內資料」分頁的當日靜態檔。只在 final 輪跑:early 輪的分點資料可能
# 還不完整,而這支是**篩選**,資料缺一塊就會選出錯的名單(不像分點增量那樣
# 只是少幾列)。放在 tools_summary 之後:彙總表是分點排行的資料來源、屬於核心,
# 不該讓這個附加功能的失敗影響到它(本檔是 set -u 不是 set -e,單一指令失敗
# 不會中止腳本,但順序還是照重要性排)。
# 不給 --date:它會取 DB 最新有資料日,休市日自然落回前一交易日、檔案已存在
# 就整批略過。--limit 40 是留餘裕;超過會印警告,不會靜默截斷。
# **每天的檔數會劇烈變動,少不代表壞掉**:固定門檻下,2026-08-31~09-16 實測
# 介於 2 到 28 檔。9/16 只有 2 檔是真的 —— 分點覆蓋率 99.6% 正常,但那天市場
# 成交放大且分散(成交額≥20 億的股票 81→90 檔)而元大絕對相抵量反而少 11%,
# 分母變大分子變小,佔量普遍掉到 8% 以下。要判斷是否異常請看覆蓋率,別看檔數。
# 舊資料不清理(使用者 2026-09-16 決定全部保留),每天約 +3 MB。
if [[ "$ROUND" != "early" ]]; then
    $PY -u demo_build.py --limit 40
fi
