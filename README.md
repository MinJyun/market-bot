...
- 統一系基金的**期貨部位**(AssetCode=GD,名目本金)一併入庫:Share 為口數、
  Amount 為名目金額;訊息以 ⚡ 常駐顯示曝險(口數+佔淨值%),增減倉走既有
  差異報告(單位以名稱含「期貨」判斷,顯示口)。其他投信尚未見期貨揭露。
...
另抓 MI_INDEX(ALLBUT0999)全部個股收盤價(`stock_close` 表,一請求全市場),
估算**大盤融資維持率** = Σ(每檔融資餘額×收盤價) ÷ 全市場融資金額 × 100%
(餘額覆蓋率 99.99%)。交易所不公布整戶維持率,此為媒體常用估算口徑(上市;
忽略現金增提擔保),趨勢參考;低於 ~150% 常伴隨斷頭賣壓。
