...
## 資料源:market_index(加權指數收盤行情)

TWSE MI_INDEX「價格指數」表,只取「發行量加權股價指數」(大盤)的收盤指數、
漲跌點數、漲跌百分比。放在「法人籌碼」訊息最前面,作為當天其他籌碼指標
(法人買賣超、融資融券等)的漲跌對照基準。TWSE 開放 JSON、無 bot 防護,
可回補歷史。
...
TWSE MI_MARGN「信用交易統計」彙總表:全市場融資(散戶多方槓桿)、融券(放空)
每日餘額(單位:張)。融資餘額增加代表散戶加碼、融券餘額增加代表放空力道
增加,是常見的散戶籌碼/情緒指標。TWSE 開放 JSON、無 bot 防護,可回補歷史。
